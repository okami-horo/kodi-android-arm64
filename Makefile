...
APP_PACKAGE_DIR = $(subst .,/,org.xbmc.kodi)
...
python: | xbmc/assets
	mkdir -p xbmc/assets/python/lib/
	cp -rfp $(PREFIX)/lib/python xbmc/assets/python/lib/
	cd xbmc/assets/python/lib/python/; rm -rf test config config- lib-dynload ; find . -name "*.so" -exec rm -f {} \;
...
libs: $(PREFIX)/lib/kodi/libkodi.so
	...
	mkdir -p xbmc/lib/$(CPU) xbmc/assets/python/lib/ xbmc/obj/local/$(CPU)
	cp -fpL $(SRCLIBS) xbmc/obj/local/$(CPU)/
	cp -fp $(PREFIX)/lib/kodi/libkodi.so xbmc/obj/local/$(CPU)/
	(test -d $(PREFIX)/lib/kodi/addons && \
	  find $(PREFIX)/lib/kodi/addons -name "*.so" -exec cp -fp {} xbmc/obj/local/$(CPU)/ \;) || true
	find $(PREFIX)/share/kodi/addons -name "*.so" -exec cp -fp {} xbmc/obj/local/$(CPU)/ \; || true
	...
	DIR=${CURDIR}; cd $(PREFIX)/lib/python/site-packages; for i in `find Cryptodome -name \*.so` ; do FN=`echo $$i | cut -c1- | tr "/" "_" | sed -e 's/\.abi[0-9]\\././'` ;cp $$i $$DIR/xbmc/obj/local/$(CPU)/$$FN ; done
...
package: libs python java
	...
	@cp xbmc/build/outputs/apk/$(BUILD_TYPE_LC)/xbmc-$(BUILD_TYPE_LC).apk $(CMAKE_SOURCE_DIR)/kodiapp-$(CPU)-$(BUILD_TYPE_LC).apk
